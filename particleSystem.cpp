...
void ParticleSystem::computeForcesAndUpdateParticles(float t)
{
	...
	{
		std::deque<Particle>::iterator itLastDead = particles.begin();
		for (std::deque<Particle>::iterator it = particles.begin(); it != particles.end(); it++)
		{
			it->update(bake_fps);
			if (it->getLife() < 0) itLastDead = it;
		}
		particles.erase(particles.begin(), itLastDead);
		bakeParticles(t);
	}
}
...
void ParticleSystem::drawParticles(float t)
{
	if (isBakedAt(t))
	{
		map<float, std::deque<Particle>>::iterator iter = bakeRec.find(t);
		for (std::deque<Particle>::iterator it = iter->second.begin(); it != iter->second.end(); it++) it->draw(m_camera);
	}
	else if (isSimulate())
		for (std::deque<Particle>::iterator it = particles.begin();it != particles.end(); it++) it->draw(m_camera);
}
